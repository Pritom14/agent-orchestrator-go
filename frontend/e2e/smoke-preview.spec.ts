...
test("P0 preview tab opens on the ao preview target @P0 @PREVIEW", async ({ page }) => {
...
test("P0 preview can be cleared @P0 @PREVIEW", async ({ page }) => {
	await installFakeAgent(page, {
		workers: [{ id: "prev", title: "Preview worker", previewUrl: "http://localhost:5173", previewRevision: 1 }],
	});
	await page.goto("/#/projects/fake-proj/sessions/prev");
...
test("P0 missing preview target surfaces an actionable error @P0 @PREVIEW", async ({ page }) => {
